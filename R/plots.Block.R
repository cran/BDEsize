#' Graphs for investigating sample size in randomized complete block design
#'
#' Three graphs in randomized complete block design are for investigating the mutual relationship between the sample size, power "1-beta" and the detectable standardized effect size "delta"
#'
#' @usage plots.Block(factor,factor.lev,order,delta_type,delta,deltao,alpha,beta,type)
...
plots.Block<-
  function(factor, factor.lev,order=1, delta_type=1 , delta , deltao=1 , alpha=0.05, beta=0.2, type )
  {
    ...
    FF<-Size.Block(factor, factor.lev,order, delta_type, delta, alpha, beta)
    (n.choose <- FF$n);
